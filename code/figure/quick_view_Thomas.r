...
for (s1 in 1:3){
	...
	axis(1, at=log10Tck('x','major'), tcl= 0.5,cex.axis=1.75) # bottom
	axis(1, at=log10Tck('x','minor'), tcl= 0.1, labels=NA) # bottom
	axis(2, at=log10Tck('y','major'), tcl= 0.5,cex.axis=1.75) 
...
#	PCF_spat=pcf3est(ppp_repart,rmax=max(f$r),nrval=1000,delta=10^(-5))
        ...
        plot(0.1,0.1,t="n",log="x",xlab=xl,ylab="g(r)",axes=F,xlim=c(10^(-4),0.1),cex.lab=2,ylim=range(f_tot$pcf[f_tot$pcf>0]),main=paste("Species=",unique_sp[s1],sep=""),cex.main=2)
        axis(1, at=log10Tck('x','major'), tcl= 0.5,cex.axis=2) # bottom
        axis(1, at=log10Tck('x','minor'), tcl= 0.1, labels=NA) # bottom
#        axis(2, at=log10Tck('y','major'), tcl= 0.5,cex.axis=1.5)
#        axis(2, at=log10Tck('y','minor'), tcl= 0.1, labels=NA) 
	axis(2,cex.axis=2)
        box()
