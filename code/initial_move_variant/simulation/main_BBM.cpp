...
extern const int num_simu=1000; //Simulation identifier
...
extern const double delta_fixed=pow(10,-5.0); //Only used if delta_spatstat==0
...
void branching_process(std::vector<basic_particle> &part_1,double proba_repro, double proba_death)
{
	...
	for(int j=0;j<size;j++)
	{
		...
		if (proba<proba_repro) //new particle at the same place, and the parent is still there
		{
			...
			tmp_part.initial_move(2*radius, Lmax); //Initial move of 2 radius of the daughter cell, in a random direction in 3D
