...
for (s in 1:length(sim)){
	...
	th_thomas=thomas_cdf(unique(f_tot$r),sigma)
	th_poisson=4/3*pi*unique(f_tot$r)^3

	plot(0.1,0.1,t="n",log="xy",xlab=xl,ylab=yl,axes=F,xlim=c(10^(-4),0.1),cex.lab=1.5,ylim=range(f_tot$K)+10^(-4),main=paste("Species=",s1," correct=",type[s],sep=""))
