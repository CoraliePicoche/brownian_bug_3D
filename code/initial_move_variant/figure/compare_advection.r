...
pdf("dominance_diatom_nano_compare_advection_simus1000.pdf",width=13)
par(mfrow=c(1,2),mar=c(4,4.5,4,4))

#Diatom
#sim_diatom=list(10,11) #Advection, no advection
#sim_diatom=list(0,1) #Advection, no advection
sim_diatom=list(1000,1001) #Advection, no advection
lim_max_diatom=25*10^(-4)*10*2

#Nano
#sim_nano=list(12,13)
#sim_nano=list(2,3)
sim_nano=list(1002,1003) #Advection, no advection
...
corresponding_names=c(expression(r[95~"%"]),expression(dominance(d[threshold])))
...
for(orga in 1:length(tot_sim)){ #Organism: diatom or nano
	for(adv in 1:length(tot_sim[orga][[1]])){ #Advection or no advection
		nb_simu=tot_sim[orga][[1]][[adv]]
        	...
        	f_tot=read.table(paste("../simulation_",nb_simu,"/lambda_K_",nb_simu,".txt",sep=""),sep=";",header=F,dec=".")
		...
		f_count=read.table(paste("../simulation_",nb_simu,"/nb_indiv_",nb_simu,".txt",sep=""),sep=";",header=F,dec=".")
